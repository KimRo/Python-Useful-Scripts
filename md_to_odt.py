# md_to_odt.py — Convert Markdown files to ODT (Open Document Text).
#
# Requires pandoc to be installed: https://pandoc.org/installing.html
#
# Usage:
#   python md_to_odt.py [folder]              interactive menu to pick a file
#   python md_to_odt.py [folder] --all        convert all files without menu
#   python md_to_odt.py --file report.md      convert a single specific file
#   python md_to_odt.py [folder] --silent     fully non-interactive (for automation)
#
...
#
# On first attempt the raw markdown is used.  If pandoc fails, md_repair.py
...
def _odt_from_text(text: str, out_path: Path) -> None:
    """Convert markdown *text* to ODT and write it to *out_path* via pandoc."""
    ...
    try:
        result = subprocess.run(
            ["pandoc", str(tmp_path), "-f", "markdown", "-t", "odt", "-o", str(out_path)],
            capture_output=True,
            text=True,
        )
        if result.returncode != 0:
            raise RuntimeError(result.stderr.strip() or "pandoc returned a non-zero exit code")
    finally:
...
def convert_file(
    ...
    output_folder: Path,
    silent: bool = False,
    fix_headings: str | None = None,
    renumber: bool | None = None,
) -> None:
    """
    Convert *md_path* to ODT.

    On first attempt the raw markdown is used.  If pandoc raises an exception,
    md_repair.repair() is called to clean the content and the conversion is
    retried once.
    ...
    """
    out_path = output_folder / (md_path.stem + ".odt")
    text = md_path.read_text(encoding="utf-8")

    try:
        _odt_from_text(text, out_path)
    ...
    try:
        _odt_from_text(repaired, out_path)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert Markdown files to ODT (Open Document Text).",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Silent mode examples (no prompts):
  python md_to_odt.py --silent                       # convert all files in current dir
  python md_to_odt.py docs/ --silent                 # convert all files in docs/
  python md_to_odt.py --file report.md --silent      # convert one specific file
  python md_to_odt.py --silent --renumber            # convert all and renumber headings
  python md_to_odt.py --silent --fix-headings insert # insert H1 when missing
""",
    )
    parser.add_argument(
        "folder",
...
_BANNER = """md_to_odt.py — Convert Markdown files to ODT (Open Document Text).

  Requires: pandoc  (https://pandoc.org/installing.html)

  Usage:
    python md_to_odt.py [folder]                       interactive menu
    python md_to_odt.py [folder] --all                 convert all files
    python md_to_odt.py --file report.md               convert a single file
    python md_to_odt.py [folder] --silent              non-interactive (automation)

  Repair options (used when conversion fails and md_repair.py is invoked):
    --fix-headings shift|insert|skip   how to handle a missing H1 heading
...
def main():
    ...
    if _folder_arg.is_file():
        # Allow: python md_to_odt.py file.md [--silent ...]
        if args.file:
